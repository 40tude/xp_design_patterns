...
impl Fsm<Whitespace> {
    ...
    fn process_char(mut self, c: char) -> Machine {
        // let mut stats = self.stats; // moved, not cloned (see below the clone in comment)
...
impl Machine {
    ...
    fn into_stats(self) -> TextStats {
        match self {
            Machine::White(Fsm { stats, .. }) | Machine::Word(Fsm { stats, .. }) | Machine::Number(Fsm { stats, .. }) => stats, // moved, not cloned
        }
    }
}
...
fn process_text(text: &str) -> TextStats {
    ...
    for c in text.chars() {
        m = m.process_char(c); // move-based, no clones
    }
    // m.stats().clone()
    m.into_stats()
}
