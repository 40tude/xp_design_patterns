...
// Your previous approach with traits would be more suitable if:
...
impl State {
    ...
    pub fn transition(self, event: Event) -> Self {
        match (self, event) {
            (State::Validated, Event::Process) => {
                println!("State: Validated -> Enriched");
                State::Enriched
            }
            (State::Enriched, Event::Process) => {
                println!("State: Enriched -> Persisted");
                State::Persisted
            }
            (State::Persisted, Event::Process) => {
...
fn main() {
    println!("--- Enum-based State Machine Demo ---");
